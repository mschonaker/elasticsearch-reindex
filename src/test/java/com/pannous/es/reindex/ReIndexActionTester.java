...
/**
 * This test will be called from the ElasticSearch and from the 'JSON' implementation. (to avoid code duplication)
 *
 * @author Peter Karich
 */
public abstract class ReIndexActionTester extends AbstractNodesTests {
    ...
    protected abstract MySearchResponse scrollSearch(String searchIndex, String searchType, String filter, int hits,
    ...
    public void reindexAll() throws Exception {
        ...
        SearchResponse sr = client.prepareSearch("tweets").addSort("count", SortOrder.ASC).execute().actionGet();
    ...
    @Test
    public void reindexAllPartial() throws Exception {
        add("oldtweets", "tweet", null, "{ \"name\" : \"hello world\", \"count\" : 1}");
        add("oldtweets", "tweet", null, "{ \"name\" : \"peter test\", \"count\" : 2}");
        refresh("oldtweets");
        assertThat(count("oldtweets"), equalTo(2L));
        int res = action.reindex(scrollSearch("oldtweets", "tweet", "{ \"term\": { \"count\" : 2} }"), "tweets",
                "tweet", false, 0, client);
    ...
    @Test
    public void reindexChilds() throws Exception {
        ...
        client.admin().indices().preparePutMapping().setIndices("oldtweets").setType("retweet")
                .setSource(
                        "{\"retweet\": { \"_parent\": { \"type\": \"tweet\" }, \"_routing\": { \"required\": true }, \"properties\": { \"name\": { \"type\": \"string\" }, \"count\": { \"type\": \"long\" } } }}")
                .execute().actionGet();
        String child = add("oldtweets", "retweet", parent,
                "{ \"name\" : \"RE: hello world\", \"count\" : 1, \"_parent\" : \"" + parent + "\"}");
        refresh("oldtweets");
        assertThat(count("oldtweets"), equalTo(2L));
        ...
        // update the mapping settings for oldtweets childs (i.e retweet type)
        client.admin().indices().preparePutMapping().setIndices("tweets").setType("retweet")
                .setSource(
                        "{\"retweet\": { \"_parent\": { \"type\": \"tweet\" }, \"_routing\": { \"required\": true }, \"properties\": { \"name\": { \"type\": \"string\" }, \"count\": { \"type\": \"long\" } } }}")
                .execute().actionGet();
        ...
        // now check if content was correctly streamed and saved
        SearchResponse parent_sr = client.prepareSearch("tweets").setTypes("tweet").addSort("count", SortOrder.ASC)
                .execute().actionGet();
        assertThat(parent_sr.getHits().hits().length, equalTo(1));
        String reindex_parent = parent_sr.getHits().hits()[0].id();

        SearchResponse child_sr = client.prepareSearch("tweets").setTypes("retweet").setRouting(reindex_parent)
                .setQuery(QueryBuilders.boolQuery().must(QueryBuilders.termQuery("_parent", reindex_parent)))
                .addSort("count", SortOrder.ASC).execute().actionGet();
        assertThat(child_sr.getHits().hits().length, equalTo(1));
    }

    @Test
    public void copyAliases() throws Exception {
        add("oldtweets", "tweet", null, "{ \"name\" : \"hello world\", \"count\" : 1}");
        ...
        Method copyAliases = action.getClass().getDeclaredMethod("copyAliases", String.class, String.class,
                Boolean.class, Client.class);
